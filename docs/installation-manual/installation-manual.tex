...
\newcommand{\boilertversion}{1.3.0}
...
value remains on the broker) and the energy is computed from the remaining
valid sensors. \code{offline} is set automatically by the broker if the
connection drops (MQTT last will).
...
A sample unit is provided in \code{deploy/boilert.service}. Adjust
\code{User}, \code{WorkingDirectory} and \code{ExecStart} to your paths, then:
...
The service restarts automatically on failure. Log verbosity is controlled by
...
\begin{lstlisting}[language=bash]
journalctl -u boilert -f                     # application logs
...
\begin{enumerate}
  ...
  \item Restart the service: \code{sudo systemctl restart boilert}.
\end{enumerate}
